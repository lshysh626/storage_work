...
const TYPE_LABEL = { short: '?�답??, essay: '?�술??, practical: '?�무?? };
const TYPE_POINTS = { short: 3, essay: 12, practical: 16 };

// ?�?�?� API Key Error UI ?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�
function buildApiKeyErrorHTML(message, retryCall) {
    const isQuota = message.includes('?�도') || message.includes('quota') || message.includes('Quota');
    return `
        <div style="padding: 1.2rem; border-radius: 10px; background: rgba(30,41,59,0.8); border: 1px solid rgba(248,113,113,0.3);">
            <div style="color: #f87171; font-weight: 700; margin-bottom: 0.8rem;">
                ${isQuota ? '?�� API ???�청 ?�도 초과' : '?�️ AI ?�결 ?�류'}
            </div>
            <p style="color: #94a3b8; font-size: 0.88rem; margin-bottom: 1rem; line-height: 1.6;">
                ${isQuota
                    ? '지�??�용중인 API ?�의 무료 ?�도가 초과?�었?�니??<br>?�래??<strong style="color:#38bdf8;">Google AI Studio</strong>?�서 발급??본인 API ?��? ?�력?�세??'
                    : message
                }
            </p>
            ${isQuota ? `
            <div style="display:flex; gap:0.5rem; margin-bottom:0.8rem;">
                <input id="quick-api-key" type="password" placeholder="AIzaSy... (???�력 ??Enter)"
...
function saveQuickApiKey(retryCall) {
    ...
    if (!key) { alert('?��? ?�력?�주?�요.'); return; }
    localStorage.setItem('gemini_api_key', key);
    // ?�정 ?�이지 ?�기??    const settingInput = document.getElementById('setting-api-key');
...
async function renderDashboard() {
    ...
    document.getElementById('stats-grid').innerHTML = `
        <div class="stat-card"><div class="label">�??�???�수</div><div class="value">${total}</div></div>
        <div class="stat-card"><div class="label">?�균 ?�수</div><div class="value">${avg}%</div></div>
        <div class="stat-card"><div class="label">최근 ?�수</div><div class="value">${last}</div></div>
    `;
        ...
        if (sessions.length === 0) {
            parsedDiv.innerHTML = `
                <div style="background:rgba(30,41,59,0.4); border:1px solid rgba(255,255,255,0.05); border-radius:16px; padding:2rem; text-align:center;">
                    ...
                    <p style="color:var(--muted); margin-top:0.8rem;">?�싱??기출 ?�료가 ?�습?�다. <strong style="color:var(--primary); cursor:pointer;" onclick="document.getElementById('sync-btn').click()">?�이???�기??/strong>�?먼�? ?�주?�요.</p>
                </div>
            `;
            return;
        }
        ...
        parsedDiv.innerHTML = `
            <div style="background:rgba(30,41,59,0.4); border:1px solid rgba(255,255,255,0.05); border-radius:16px; padding:2rem;">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1.5rem;">
                    <h3 style="font-weight:800; font-size:1.2rem;">?�� ?�싱??기출 ?�료</h3>
                    <div style="display:flex; gap:1rem; align-items:center;">
                        <span style="color:var(--muted); font-size:0.9rem;">${sessions.length}�??�차</span>
                        <span style="background:rgba(56,189,248,0.1); color:var(--primary); padding:0.3rem 0.8rem; border-radius:6px; font-weight:800; font-size:0.9rem; border:1px solid rgba(56,189,248,0.2);">�?${totalQuestions}문항</span>
                    </div>
                </div>
...
function renderStats() {
    ...
    if (history.length === 0) {
        container.innerHTML = `
            <div style="text-align: center; padding: 4rem 2rem;">
                ...
                <h2 style="color: var(--muted); font-weight: 700; margin-bottom: 1rem;">?�직 ?�습 기록???�습?�다</h2>
                <p style="color: var(--muted); font-size: 1.1rem;">기출 ?�기에??문제�??�?�보?�요!</p>
            </div>
        `;
        return;
    }
    ...
    const rows = [...history].reverse().slice(0, 20).map((r, i) => {
        ...
        return `
            <tr style="border-bottom: 1px solid rgba(255,255,255,0.03); cursor: pointer; transition: background 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.02)'" onmouseout="this.style.background='transparent'" onclick="document.getElementById('details-${r.id}').style.display = document.getElementById('details-${r.id}').style.display === 'none' ? 'table-row' : 'none'">
                ...
                <td style="padding:1rem; text-align:center;">${r.totalScore ?? 0} / ${r.maxScore ?? 0}??/td>
                    ...
                    <div style="display:flex; flex-direction:column; gap:0.8rem; max-height:400px; overflow-y:auto; padding-right:0.5rem;">
                        ${(r.details || []).map((d, idx) => `
                            <div style="background:rgba(255,255,255,0.03); padding:1rem; border-radius:8px; border-left:4px solid ${d.is_correct ? '#10b981' : '#f87171'}">
                                ...
                                <div style="font-size:0.95rem; margin-bottom:0.4rem; color:#e2e8f0;"><span style="color:var(--muted); margin-right:0.4rem;">???��?:</span> ${d.user_answer}</div>
                                <div style="font-size:0.95rem; margin-bottom:0.6rem; color:#e2e8f0;"><span style="color:#38bdf8; margin-right:0.4rem;">모범 ?�안:</span> ${d.correct_answer}</div>
                                <div style="font-size:0.9rem; color:${d.is_correct ? '#10b981' : '#f87171'}; font-weight:bold;">[${d.score}/${d.points}?? ${d.feedback}</div>
                            </div>
                        `).join('')}
                    </div>
                </td>
    ...
    container.innerHTML = `
        ...
        <div style="background:rgba(30,41,59,0.4); border:1px solid rgba(255,255,255,0.05); border-radius:16px; padding:2rem; margin-bottom:2.5rem;">
            <h3 style="margin-bottom:1.5rem; font-weight:800; font-size:1.2rem;">?�� 최근 ?�수 추이 (최�? 10??</h3>
            <div style="display:flex; align-items:flex-end; gap:0.5rem; height:200px; padding:0 0.5rem;">
                ${chartBars}
            </div>
            <div style="text-align:center; margin-top:1rem; color:var(--muted); font-size:0.85rem;">60% ?�상 <span style="color:#10b981;">??/span> · 40~59% <span style="color:#f59e0b;">??/span> · 40% 미만 <span style="color:#f87171;">??/span></div>
        </div>

        <!-- Cumulative -->
        <div style="background:rgba(30,41,59,0.4); border:1px solid rgba(255,255,255,0.05); border-radius:16px; padding:2rem; margin-bottom:2.5rem;">
            <h3 style="margin-bottom:1.2rem; font-weight:800; font-size:1.2rem;">?�� ?�적 ?�적</h3>
            <div style="display:flex; align-items:center; gap:2rem;">
                <div style="flex:1;">
                    <div style="display:flex; justify-content:space-between; margin-bottom:0.5rem;">
                        <span style="color:var(--muted);">?�적 ?�득</span>
                        <span style="font-weight:800;">${totalScore} / ${totalMax}??/span>
                    </div>
        ...
        <div style="background:rgba(30,41,59,0.4); border:1px solid rgba(255,255,255,0.05); border-radius:16px; overflow:hidden; margin-bottom:2rem;">
            <h3 style="padding:1.5rem 2rem 1rem; font-weight:800; font-size:1.2rem;">?�� ?�??기록 (최근 20�?</h3>
            <table style="width:100%; border-collapse:collapse;">
                <thead>
                    <tr style="border-bottom:1px solid rgba(255,255,255,0.08);">
                        <th style="padding:0.8rem 1rem; text-align:left; color:var(--muted); font-size:0.85rem;">#</th>
                        <th style="padding:0.8rem 1rem; text-align:left; color:var(--muted); font-size:0.85rem;">?�짜</th>
                        <th style="padding:0.8rem 1rem; text-align:center; color:var(--muted); font-size:0.85rem;">?�수</th>
                        <th style="padding:0.8rem 1rem; text-align:center; color:var(--muted); font-size:0.85rem;">?�답�?/th>
                    </tr>
                </thead>
                <tbody>${rows}</tbody>
            </table>
...
function toggleTypeSelection() {
    ...
    state.subMode = 'type';
    sub.style.display = 'block'; // CSS??grid ?�정??무시?�여 찌그?�짐 방�?
    sub.classList.remove('hidden');
    sub.innerHTML = `
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1rem;">
            <div class="item-row" id="type-row-short" style="cursor: pointer;" onclick="selectType('short', 12, '?�답??)">
                <div class="text"><strong>?�답??/strong><span>3??문제</span></div>
            </div>
            <div class="item-row" id="type-row-essay" style="cursor: pointer;" onclick="selectType('essay', 4, '?�술??)">
                <div class="text"><strong>?�술??/strong><span>12??문제</span></div>
            </div>
            <div class="item-row" id="type-row-practical" style="cursor: pointer;" onclick="selectType('practical', 2, '?�무??)">
                <div class="text"><strong>?�무??/strong><span>16??문제</span></div>
            </div>
        </div>
        ...
        <div id="type-action-area" style="display: none; margin-top: 1.5rem; animation: fadeIn 0.3s ease;">
            <div style="max-width: 420px; margin: 0 auto; background: rgba(30, 41, 59, 0.9); border: 1px solid var(--primary); padding: 1.5rem 2rem; border-radius: 16px; box-shadow: 0 10px 25px rgba(0, 0, 0, 0.5), 0 0 20px rgba(56, 189, 248, 0.15); text-align: center;">
                <div style="margin-bottom: 1.5rem; color: #fff; font-size: 1.15rem;">
                    <strong id="selected-type-name" style="color: var(--primary); font-size: 1.3rem;">?�형</strong>
                    <span style="color: #cbd5e1; font-weight: 500;"> �?문제�??�시겠습?�까?</span>
                </div>
                <div style="display: flex; gap: 1rem; justify-content: center;">
                    ...
                    <button id="global-type-start-btn" style="flex: 1; background: linear-gradient(135deg, #0ea5e9, #38bdf8); color: #fff; border: none; border-radius: 10px; font-weight: 800; font-size: 1.2rem; cursor: pointer; transition: 0.2s; box-shadow: 0 4px 12px rgba(56, 189, 248, 0.3);" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='none'">?�작?�기</button>
                </div>
            </div>
        </div>
    `;
...
async function toggleSessionSelection() {
    ...
    sub.classList.remove('hidden');
    sub.innerHTML = '<div style="color:var(--muted);padding:1rem">불러?�는 �?..</div>';
    try {
        const res = await fetch('./data/sessions.json');
        const sessions = await res.json();
        if (sessions.length === 0) {
            sub.innerHTML = '<div style="color:var(--muted);padding:1rem">?�기?????�차가 ?��??�니??</div>';
            return;
        }
        ...
        sub.innerHTML = '<div style="color:#f87171;padding:1rem">?�버 ?�결 ?�류</div>';
    }
}
...
async function startTypeQuiz(type) {
    ...
    launchQuiz(questions, `?�� ${TYPE_LABEL[type] || type} ?�덤 모의고사 (${questions.length}??`);
}
...
function renderQuestion() {
    ...
    document.getElementById('q-points').textContent = `${q.points ?? TYPE_POINTS[q.type] ?? 0}??;
    ...
    if (expContainer) expContainer.innerHTML = `
        <div style="padding: 1.5rem;">
            <button onclick="requestExplanation(${state.index})" id="explain-btn"
                ...
                ?�� ?�세 ?�설 ?�기 <span style="font-size:0.78rem; opacity:0.6; font-weight:400;">(Ctrl+Alt+T)</span>
            </button>
        </div>
    `;
    ...
    if (state.quizMode === 'study') {
        container.innerHTML = `
            <div style="background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(16, 185, 129, 0.3); border-radius: 8px; padding: 1.5rem; margin-top: 1rem;">
                <strong style="color: var(--primary); display: block; margin-bottom: 0.8rem; font-size: 1.1rem;">?�� ?�답 (?�습 모드)</strong>
                <span style="color: #fff; line-height: 1.6; white-space: pre-wrap;">${q.answer}</span>
            </div>
        `;
        nextBtn.textContent = '?�음 문제 ??(Ctrl+Enter)';
        nextBtn.style.background = '';
    } else {
        const matchText1 = q.question.match(/\(\s*[A-Za-z가-?�ㄱ-??\s*\)/g) || [];
        ...
        if (blanks.length > 0) {
            blanks.forEach(label => {
                ...
                row.innerHTML = `
                    ...
                    <input class="ans-input" type="text" data-label="${label}" placeholder="?�안 ?�력...">
                `;
                container.appendChild(row);
            });
        } else {
            container.innerHTML = `<textarea class="textarea-ans" placeholder="?�안???�력?�세??.."></textarea>`;
        }
        
        if (state.bulkScoring) {
            if (state.index === state.questions.length - 1) {
                nextBtn.textContent = '?�출 �?채점 ?? (Ctrl+Enter)';
            ...
            nextBtn.textContent = '?�출 (Ctrl+Enter)';
            nextBtn.style.background = '';
        }
...
async function submitAnswer() {
    ...
    if (!userAnswer?.trim() || userAnswer === '미입??) {
        userAnswer = '미입??;
    }
    ...
    fb.innerHTML = '<span style="color:var(--primary)">채점 �?.. ??/span>';
    ...
    try {
        ...
        fb.innerHTML = `
            <div style="border-left: 4px solid ${result.is_correct ? 'var(--primary)' : '#f87171'}; padding-left: 1rem;">
                <strong>${result.is_correct ? '???�답' : '?�️ ?�답/부분점??} (${result.score}??</strong>
                <div style="margin-top:0.8rem; background: rgba(0,0,0,0.2); padding: 0.8rem; border-radius: 6px;">
                    <strong style="color:var(--primary);">모범 ?�안:</strong> 
                    <span style="color:#fff;">${q.answer}</span>
                </div>
                <p style="margin-top:0.8rem; color:var(--muted); white-space:pre-wrap">${result.feedback}</p>
                <button class="btn-next" onclick="nextQuestion()" style="margin-top:1.5rem; width:100%">?�음 문제 ??(Ctrl+Enter)</button>
            </div>
        `;
    } catch (err) {
        fb.innerHTML = `<span style="color:#f87171">?�류: ${err.message}</span>`;
        state.submitting = false;
    }
}
...
async function submitBulkAnswers() {
    ...
    container.innerHTML = '<div style="text-align:center; padding: 4rem; font-size: 1.5rem; color: var(--primary);">채점 중입?�다... ?�시�?기다?�주?�요 ??</div>';
    ...
    try {
        ...
        container.innerHTML = `<div style="color:#f87171; padding: 2rem;">?�류: ${e.message}</div>`;
    }
}
...
function showSessionResult() {
        ...
        html += `
            <div style="background: rgba(15,23,42,0.6); border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 1.5rem; margin-bottom: 1.5rem; border-left: 4px solid ${d.is_correct ? 'var(--primary)' : '#f87171'}">
                <div style="display:flex; justify-content:space-between; margin-bottom: 1rem;">
                    ...
                    <span style="font-size: 1.1rem; font-weight: 800; color: ${d.is_correct ? 'var(--primary)' : '#f87171'}">${d.score} / ${d.points}??/span>
                </div>
                <p style="color: #cbd5e1; margin-bottom: 1rem; line-height: 1.5;">${d.question}</p>
                <div style="background: rgba(0,0,0,0.2); padding: 1rem; border-radius: 8px; margin-bottom: 1rem;">
                    <strong style="color: var(--muted); display: block; margin-bottom: 0.5rem;">???�안:</strong>
                    ...
                    <div style="margin-top: 0.8rem; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 0.8rem;">
                        <strong style="color: var(--primary); display: block; margin-bottom: 0.5rem;">모범 ?�안:</strong>
                        <span style="color: #fff;">${d.correct_answer}</span>
                    </div>
                </div>
                <div>
                    <strong style="color: var(--muted); display: block; margin-bottom: 0.5rem;">AI ?�드�?</strong>
    ...
    document.getElementById('final-score').textContent = `총점: ${session.totalScore} / ${session.maxScore}??;
...
async function requestExplanation(index) {
    ...
    // Reset Chat
    chatMessages.innerHTML = '<div class="chat-message ai-message">??문제???�??궁금???�을 질문?�세??</div>';
    
    // (?�동?�로 ?�널???��? ?�고 백그?�운?�에???�용�??�데?�트?�니??
    ...
    const headerHTML = `
        <div id="exp-header" onclick="toggleExplanation()" style="
            ...
            user-select: none;
        ">
            <strong style="color: #60a5fa; font-size: 1.05rem;">?�� ?�세 ?�설</strong>
            <span id="exp-toggle-icon" style="color: #60a5fa; font-size: 0.85rem; opacity: 0.7;">&#9650; ?�기</span>
        </div>
        <div id="exp-body">
    `;
...
async function sendChatMessage() {
    ...
    try {
        ...
        await geminiChatStream(
            ...
            (status) => {
                aiMsgDiv.textContent = status; // 카운?�다???�시
                aiMsgDiv.style.color = '#f59e0b';
            }
        );
        aiMsgDiv.style.color = ''; // ?�상 ?�상 복원
    } catch (e) {
...
window.addEventListener('keydown', e => {
    if (state.view === 'quiz-view' && e.ctrlKey) {
        ...
        if (e.key === 'Enter') {
            ...
            e.preventDefault();
            const fb = document.getElementById('feedback-area');
            if (!fb.classList.contains('hidden') && !fb.textContent.includes('채점 �?)) {
                nextQuestion();
            } else {
                submitAnswer();
            }
...
async function syncData() {
    ...
    if (!key) {
        alert('API ?��? ?�정?�어 ?��? ?�습?�다. ?�정 메뉴?�서 ?��? 먼�? ?�력?�주?�요.');
        return;
    }
    ...
    try {
        ...
        if (data.errors && data.errors.length > 0) {
            ...
            alert(`?�기???�료 (?��? ?�류 발생): ${data.processed.length}�??�션 ?�공`);
        } else {
            alert(`?�기???�료: ${data.processed.length}�??�션 처리??);
        }
        loadSessions(); // ?�션 목록 ?�로고침
        renderDashboard(); // ?�?�보??갱신
    } catch (e) {
        console.error(e);
        alert('?�기???�패: ?�버 ?�결 ?�태�??�인?�세??');
    } finally {
